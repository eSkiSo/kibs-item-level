...
function AddOn:OnInitialize()
    ...
    self:RegisterChatCommand("kib", function(input)
        if not input or input:trim() == "" then
            Settings.OpenToCategory(self.configPanel.name)

            SettingsPanel.AddOnsTab:Click("LeftButton", true)

            for _, button in pairs(self:FindFrames("SettingsPanel.CategoryList")) do
    ...
    self.configPanel.okay = function()
        ...
        local wasOff = self.db.profile.Inspection
        self.db.profile.Inspection = self.configPanel.controls.Inspection:GetChecked()
        if self.db.profile.Inspection and not wasOff then
            -- load inspector frame if setting is changed
            self.inspectorSlotIconManager = addonNamespace.SlotIconManager:new(addonNamespace.InspectionFrameAdapter:new())
        end
